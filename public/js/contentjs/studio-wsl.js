...
var nodes = new vis.DataSet([{
    ...
    x: "-675",
    y: "-875",
    description: "[Note: If this activity is done in class, students should not do Unit WS Extension D.]  In this activity, students read about earthquake waves and then determine the minimum number of seismometers needed to determine the location of an earthquake's epicenter within a small area. They work with graphs of data to determine the time difference in the arrival of S- and P-waves at each of a set of seismometers. From this they determine the distance of the epicenter from each seismometer and then use a web-based tool to draw circles on a map to represent this data. Finally, they determine how many seismometers are actually needed and check their idea using another set of data."
}, {
    id: 7,
    label: 'Ext A: How \nDo Waves \nMove?',
    x: "-575",
...
var nodes2 = new vis.DataSet([{
    ...
    x: "-675",
    y: "-900",
    description: "In this activity, students design a retroreflector—a device that will reflect light back in the opposite direction over a range of incoming angles. The motivation for this design relates to accurately measuring the distance to the Moon with laser pulses. They first work with diagrams to investigate arrangements of two mirrors joined at different angles to each other, drawing light rays according to the law of reflection to see if such arrangements solve the design problem. They should conclude that arranging the mirrors at 90° to each other will work. They then test their theoretical results, first using real mirrors and a collimated flashlight, and then by looking at their own reflection. Finally, they are challenged to add a third mirror to see if they can build a three-dimensional retroreflector (often called a 'corner cube').",
}, {
    id: 8,
